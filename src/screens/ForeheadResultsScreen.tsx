...
import './ForeheadResultsScreen.css';

interface ForeheadResultsScreenProps {
...
export function ForeheadResultsScreen({ players, onPlayAgain, onNewGame }: ForeheadResultsScreenProps) {
  ...
  return (
    <div className="screen results-screen">
      <div className="winner-banner card forehead-won">
        {winners.length > 0 &&
        ...
        <span className="winner-label">
          {winners.length === 0
            ? t('foreheadResults.noWinner')
            : winners.length === 1
              ? t('foreheadResults.winner', { name: winners[0].name })
              : t('foreheadResults.tie', { names: winners.map((w) => w.name).join(', ') })}
        </span>
      </div>

      <section className="results-section">
        <h2>{t('foreheadResults.scoresTitle')}</h2>
        <ul className="roster-list">
          {ranked.map((p) => (
            <li key={p.id} className={`roster-row${p.score === maxScore && maxScore > 0 ? ' is-leader' : ''}`}>
              <span>{p.name}</span>
              <span className="roster-tag forehead-results-score-tag">{tp('foreheadResults.points', p.score)}</span>
            </li>
          ))}
        </ul>
      </section>
